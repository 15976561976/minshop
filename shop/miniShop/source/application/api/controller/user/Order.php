...
class Order extends Controller
{
    ...
    public function cancel($order_id)
    {
        ...
        if ($model->cancel()) {
            //清除分销订单数据
            $distOrderModel = new DistOrder;
            $distOrderModel->removeDistOrder($this->user['user_id'], $order_id);
    ...
    public function receipt($order_id)
    {
        ...
        if ($model->receipt()) {
            //更新分销会员可提现金额
            DistUserModel::amountCalculation($this->user['user_id'], $order_id, 'complete');
            return $this->renderSuccess();
        }
        return $this->renderError($model->getError());
    }
